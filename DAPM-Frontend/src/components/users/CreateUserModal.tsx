...
const CreateUserModal: FC<Props> = ({ isOpen, onClose }) => {
    const { formState, displayRoles, handleRoleStringChange, handleCreateUser, roles, isLoading, setFormState } = useUserCreation();
        ...
        <Modal open={isOpen} onClose={onClose}>
            <form onSubmit={e => { e.preventDefault(); handleCreateUser(); }}>
                <Box sx={style}>
                    <h1>Create User</h1>
                    <Input
                        required value={formState.user.username}
                        onChange={e => setFormState(prev => ({ ...prev, user: { ...prev.user, username: e.target.value, credentials: [{ ...prev.user.credentials[0], value: e.target.value }] } }))}
                    ...
                    <Select multiple
                        ...
                        onChange={e => handleRoleStringChange(e)} input={<OutlinedInput />}
                    ...
                    <Box sx={{ display: "flex", height: "3em", flexDirection: "row", justifyContent: "space-between", width: "100%" }}>
                        <Button sx={{ bgcolor: "primary.main", color: "white", width: "10em" }} type="submit">{!isLoading ? "Create" : <Spinner />}</Button>
                        <FormControlLabel
                            control={<Checkbox onChange={e => setFormState(x => ({ ...x, user: { ...x.user, enabled: e.target.checked } }))} checked={formState.user.enabled} />}
                            label="Enabled" />
                    </Box>
                </Box>
            </form>
